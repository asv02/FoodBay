...
router.post('/createuser', [
    ...
    body('name').isLength({ min: 3 })//this is for validation of data.
], async (req, res) => {
    let success = false
    ...
    let securePass = await bcrypt.hash(req.body.password, salt);//give me hash of password with the help of salt.
    try {
        await User.create({
            name: req.body.name,
